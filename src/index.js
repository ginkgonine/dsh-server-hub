...
async function probePort(port) {
  ...
  try {
    ...
    return isDshResponse(response.status, body)
      ? { id: `auto-${port}`, port, label: `DSH :${port}`, url, source: 'auto' }
      : null
  } catch {
    return null
  }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const instances = await detectDshInstances(discovery.ports)
        sendJson(res, 200, {
          instances,
          scannedPorts: discovery.ports.length,
          discovery: discovery.source,
          warning: discovery.ports.length === 0
            ? '没有读到本机监听端口；仍可手动添加端口。'
            : instances.length === 0
