window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      .dsh-hub-stage { position: relative; min-width: 0; min-height: 0; background: color-mix(in srgb, currentColor 2%, transparent); }
      .dsh-hub-frame { position: absolute; inset: 0; width: 100%; height: 100%; border: 0; display: none; background: white; }
      .dsh-hub-frame.is-active { display: block; }
      .dsh-hub-empty { position: absolute; inset: 0; display: grid; place-items: center; padding: 28px; text-align: center; }
    ...
    function makeInstance(port, source, preferences) {
      ...
      return {
        ...
        url,
        source,
        label: preferences.names[String(port)] || `DSH :${port}`,
      }
    }
    ...
    function HubPanel(props) {
      ...
      const scan = async () => {
        ...
        try {
          ...
          const automaticPorts = Array.isArray(result.instances)
            ? result.instances.map((item) => Number(item.port)).filter((port) => Number.isInteger(port) && port > 0 && port <= 65535)
            : []
          const autoSet = new Set(automaticPorts)
          const merged = automaticPorts.map((port) => makeInstance(port, 'auto', preferences))
          ...
          setStatus(result.warning || `已发现 ${automaticPorts.length} 个 DSH`)
        } catch (error) {
      ...
      const beginRename = () => {
        ...
        setRenameValue(preferences.names[String(selected.port)] || '')
      }
      ...
      const finishRename = () => {
        ...
        if (value) names[String(target.port)] = value
        else delete names[String(target.port)]
        const nextPreferences = { names, manualPorts: preferences.manualPorts }
        replacePreferences(nextPreferences)
        setInstances(instances.map((item) => item.port === target.port
      ...
      const tabs = instances.map((item) => React.createElement('button', {
        key: item.id,
        type: 'button',
        className: `dsh-hub-tab${selectedId === item.id ? ' is-active' : ''}`,
      ...
      const frames = instances.map((item) => React.createElement('iframe', {
        key: item.id,
        className: `dsh-hub-frame${selectedId === item.id ? ' is-active' : ''}`,
        src: item.url,
        title: item.label,
